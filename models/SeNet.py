...
class SeNet(nn.Module):
    def __init__(self, in_channel=1, out_channel=2 ,training=True):
        ...
        self.SeAttention1 = nn.Sequential(
        ...
        self.SeAttention2 = nn.Sequential(
            nn.AdaptiveAvgPool3d(1),
            nn.Conv3d(32, 8, 1),
            nn.ReLU(inplace=True),
            nn.Conv3d(8, 32, 1),
        ...
        self.SeAttention3 = nn.Sequential(
            nn.AdaptiveAvgPool3d(1),
            nn.Conv3d(64, 16, 1),
            nn.ReLU(inplace=True),
            nn.Conv3d(16, 64, 1),
        ...
        self.SeAttention4 = nn.Sequential(
            nn.AdaptiveAvgPool3d(1),
            nn.Conv3d(128, 32, 1),
            nn.ReLU(inplace=True),
            nn.Conv3d(32, 128, 1),
    ...
    def forward(self, inputs):
        ...
        attention1 = self.SeAttention1(long_range1)
        long_range1 = long_range1 * attention1

        short_range1 = self.down_conv1(long_range1)

        long_range2 = self.encoder_stage2(short_range1)
        attention2 = self.SeAttention2(long_range2)
        long_range2 = long_range2 * attention2 + short_range1
        long_range2 = F.dropout(long_range2, self.dorp_rate, self.training)

        short_range2 = self.down_conv2(long_range2)

        long_range3 = self.encoder_stage3(short_range2)
        attention3 = self.SeAttention3(long_range3)
        long_range3 = long_range3 * attention3 + short_range2
        long_range3 = F.dropout(long_range3, self.dorp_rate, self.training)

        short_range3 = self.down_conv3(long_range3)

        long_range4 = self.encoder_stage4(short_range3)
        attention4 = self.SeAttention4(long_range4)
        long_range4 = long_range4 * attention4 + short_range3
...
if __name__ == "__main__":
    ...
    model = SeNet(in_channel=4, out_channel=4,training=True).to(device)
    input = torch.ones([2,4,256,256]).to(device)
