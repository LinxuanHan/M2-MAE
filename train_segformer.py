...
from models import SegFormer, SwinUnet
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(dataset=Train_Dataset(args), batch_size=args.batch_size, num_workers=args.n_threads,
                              shuffle=True)
    val_loader = DataLoader(dataset=Val_Dataset(args), batch_size=128, num_workers=args.n_threads, shuffle=False)
    # model = SegFormer.SwinTransformer(in_chans=4,
    #                         patch_size=4,
    #                         window_size=12,
    #                         embed_dim=192,
    #                         depths=(2, 2, 2, 2),
    ...
    model = SwinUnet.SwinUnet(img_size=224, num_classes=4).to(device)
