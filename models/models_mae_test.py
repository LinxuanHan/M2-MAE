...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        return x, mask, ids_restore,mask_img
    ...
    def forward(self, imgs, mask_ratio=0.75):
        latent, mask, ids_restore, mask_img = self.forward_encoder(imgs, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        loss = self.forward_loss(imgs, pred, mask)
        pred_img = self.unpatchify(pred)

        return loss,pred_img,ids_restore,mask,imgs
...
if __name__ == "__main__":
    ...
    input = torch.ones([2,4,240,240]).to(device)
