...
class UNETR(nn.Module):
    ...
    def forward(self, x_in):
        x, hidden_states_out = self.vit_1(x_in)
        # print(x.shape)
        enc1 = self.encoder1(x_in)
        # x2 = torch.cat([hidden_states_out_1[0],hidden_states_out_2[0]],dim=1)
        x2 = hidden_states_out[0]
        enc2 = self.encoder2(self.proj_feat(x2, self.hidden_size, self.feat_size))
        # x3 = torch.cat([hidden_states_out_1[1],hidden_states_out_2[1]],dim=1)
        x3 = hidden_states_out[1]
        enc3 = self.encoder3(self.proj_feat(x3, self.hidden_size, self.feat_size))
        # x4 = torch.cat([hidden_states_out_1[2],hidden_states_out_2[2]],dim=1)
        ...
        if self.training:
            ...
            # print(dec4.shape,self.hidden_size)
            # dec4 = self.output2(dec4)
            return output
            # return dec2,dec3,dec4,output
        else:
            return output
