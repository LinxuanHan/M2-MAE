...
class PatchEmbeddingBlock(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embeddings_1: nn.Module
        if self.pos_embed == "conv":
            self.patch_embeddings_1 = Conv[Conv.CONV, spatial_dims](
                in_channels=in_channels, out_channels=hidden_size, kernel_size=patch_size, stride=patch_size
            )
        self.patch_embeddings_2: nn.Module
        if self.pos_embed == "conv":
            self.patch_embeddings_2 = Conv[Conv.CONV, spatial_dims](
                in_channels=in_channels, out_channels=hidden_size, kernel_size=patch_size, stride=patch_size
            )
        self.patch_embeddings_3: nn.Module
        if self.pos_embed == "conv":
            self.patch_embeddings_3 = Conv[Conv.CONV, spatial_dims](
                in_channels=in_channels, out_channels=hidden_size, kernel_size=patch_size, stride=patch_size
            )
        self.patch_embeddings_4: nn.Module
        if self.pos_embed == "conv":
            self.patch_embeddings_4 = Conv[Conv.CONV, spatial_dims](
                in_channels=in_channels, out_channels=hidden_size, kernel_size=patch_size, stride=patch_size
            )
        self.position_embeddings = nn.Parameter(torch.zeros(1, self.n_patches*4, hidden_size))
    ...
    def forward(self, x):
        x_1 = self.patch_embeddings_1(x[:, 0:1])
        x_2 = self.patch_embeddings_2(x[:, 1:2])
        x_3 = self.patch_embeddings_3(x[:, 2:3])
        x_4 = self.patch_embeddings_4(x[:, 3:4])
        if self.pos_embed == "conv":
            x_1 = x_1.flatten(2).transpose(-1, -2)
            x_2 = x_2.flatten(2).transpose(-1, -2)
            x_3 = x_3.flatten(2).transpose(-1, -2)
            x_4 = x_4.flatten(2).transpose(-1, -2)
